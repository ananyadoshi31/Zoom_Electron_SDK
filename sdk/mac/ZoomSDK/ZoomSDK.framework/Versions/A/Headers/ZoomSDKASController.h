...
@interface ZoomSDKShareInfo: NSObject
{
    ...
    CGWindowID    _windowID; //Specify the APP that user wants to share. Available only for _shareType = ZoomSDKShareContentType_AS.
 ...
 * @param userId Specify the sharing user ID that you want to switch zoom ratio.
 * @param zoomRatio Specify the size you want to set.default is ZoomSDKShareViewZoomRatio_50,which means share content will zoom-out to 50%.
 * @return If the function succeeds,will return ZoomSDKError_Success. otherwise failed.
 */
- (ZoomSDKError)switchZoomRatioWhenViewShare:(unsigned int)userId zoomRatio:(ZoomSDKShareViewZoomRatio)zoomRatio;

/**
 * @brief Enable follow presenter's pointer by the function when watching the share on the specified view.
 * @param userId Specify the sharing user ID that you want to follow his pointer.
 * @param enable YES indicates to enable. otherwise not.
 * @return If the function succeeds,will return ZoomSDKError_Success. otherwise failed.
 */
- (ZoomSDKError)enableFollowPresenterPointerWhenViewShare:(unsigned int)userId enable:(BOOL)enable;

/**
 * @brief Determine if the follow presenter's pointer can be enabled when watching the share on the specified view.
 * @param userID Specify the sharing user ID that you want to follow his pointer.
 * @param bCan YES indicates that the pointer can be enable. NO indicates that it can’t.
 * @return If the function succeeds, it will return ZoomSDKError_Success, otherwise it fails.
*/
