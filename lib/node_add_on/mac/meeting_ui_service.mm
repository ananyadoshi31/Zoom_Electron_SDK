...
ZNSDKError ZMeetingUICtrlWrap::ChangeFloatoActiveSpkVideoSize(ZNSDKFloatVideoType type)
{
    ...
    if (type == ZN_FLOATVIDEO_Large){
        ZoomSDKError ret = [controller switchFloatVideoToWallMode];
        return Help_type.ZoomSDKErrorType(ret);
    }
    if (type == ZN_FLOATVIDEO_Small){
        ZoomSDKError ret = [controller switchFloatVideoToMinimizeMode];
        return Help_type.ZoomSDKErrorType(ret);
    }
    return ZNSDKERR_SERVICE_FAILED;
}
...
ZNSDKError ZMeetingUICtrlWrap::GetWallViewPageInfo(ZNVideoWallPageInfoParam &param)
{
    ...
    int currentPage = 0;
    int totalPage = 0;
    ZoomSDKError ret = [controller getWallViewPageInfo:&currentPage TotalPageNum:&totalPage];
    NSString *str_currentPage = [NSString stringWithFormat:@"%d",currentPage];
    NSString *str_totalPage = [NSString stringWithFormat:@"%d",totalPage];
    param.currentPage = [str_currentPage UTF8String];
    param.totalPages  = [str_totalPage UTF8String];
    nativeErrorTypeHelp  Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
...
ZNSDKError ZMeetingUICtrlWrap::SetMeetingTopic(ZoomSTRING meetingtopic)
{
    ...
    if (meetingtopic.empty()) {
        return ZNSDKERR_INVALID_PARAMETER;
    }
    NSString *topic = [NSString stringWithCString:meetingtopic.c_str() encoding:NSUTF8StringEncoding];
    ZoomSDKError ret = [controller setMeetingTopicOnMeetingInfo:topic];
    nativeErrorTypeHelp  Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
ZNSDKError ZMeetingUICtrlWrap::SetCustomizedCloudRecordingMgrUrl(ZoomSTRING crmURL)
{
    ...
    if (!recording)
        return ZNSDKERR_SERVICE_FAILED;
    NSString *url = [NSString stringWithCString:crmURL.c_str() encoding:NSUTF8StringEncoding];
    ZoomSDKError ret = [recording setCustomizedCloudRecordingMgrUrl:url];
    nativeErrorTypeHelp  Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
...
ZNSDKError ZMeetingUICtrlWrap::SetCustomizedInvitationDomain(ZoomSTRING invitation_domain)
{
    ...
    NSString *domain = [NSString stringWithCString:invitation_domain.c_str() encoding:NSUTF8StringEncoding];
    ZoomSDKError ret = [controller setCustomizedInvitationDomain:domain];
    nativeErrorTypeHelp  Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
...
ZNSDKError ZMeetingUICtrlWrap::SetCustomizedPollingUrl(ZoomSTRING pollingURL, bool bCreate)
{
    ...
    NSString *pollingStr = [NSString stringWithCString:pollingURL.c_str() encoding:NSUTF8StringEncoding];
    ZoomSDKError ret = [controller setCustomizedPollingUrl:pollingStr bCreate:bCreate];
    nativeErrorTypeHelp  Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
