...
void ZoomNodeMeetingConfigCtrlWrap::EnableInviteLinkOnMeetingUI(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	ZNSDKError err = ZNSDKERR_SUCCESS;
	do
	{
		com::electron::sdk::proto::EnableInviteLinkOnMeetingUIParams proto_params;
		if (!SetProtoParam<com::electron::sdk::proto::EnableInviteLinkOnMeetingUIParams >(args, proto_params))
		{
			err = ZNSDKERR_INVALID_PARAMETER;
			break;
		}
		...
		err = ZNSDKERR_SUCCESS;
		_g_native_wrap.GetMeetingServiceWrap().GetMeetingConfigCtrl().EnableInviteLinkOnMeetingUI(_bEnable);
	} while (false);
