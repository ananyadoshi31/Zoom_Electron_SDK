...
ZNUserInfomation ZMeetingParticipantsWrap::GetUserInfomationByUserID(unsigned int userid)
{
	...
	else
	{
		...
		zn_UserInfomation.customerKey = m_user_info.GetCustomerKey();
		zn_UserInfomation.userInfoType = ZN_REAL_USERINFO;
		zn_UserInfomation.isRawLiveStreaming = m_user_info.IsRawLiveStreaming();
		zn_UserInfomation.HasRawLiveStreamPrivilege = m_user_info.HasRawLiveStreamPrivilege();
	}
...
ZoomSTRING ZUserInfoWrap::GetCustomerKey()
{

	ZoomSTRING zn_participant_id;
	if (m_pUserInfo)
	{
		const wchar_t* temp_partiipant_id = m_pUserInfo->GetCustomerKey();
		if (temp_partiipant_id)
			zn_participant_id = temp_partiipant_id;
	}
	return zn_participant_id;
}
bool ZUserInfoWrap::IsRawLiveStreaming()
{
	bool zn_isRawLiveStreaming = false;

	if (m_pUserInfo)
	{
		zn_isRawLiveStreaming = m_pUserInfo->IsRawLiveStreaming();
	}
	return zn_isRawLiveStreaming;
}
bool ZUserInfoWrap::HasRawLiveStreamPrivilege()
{
	bool zn_HasRawLiveStreamPrivilege = false;

	if (m_pUserInfo)
	{
		zn_HasRawLiveStreamPrivilege = m_pUserInfo->HasRawLiveStreamPrivilege();
	}
	return zn_HasRawLiveStreamPrivilege;
}