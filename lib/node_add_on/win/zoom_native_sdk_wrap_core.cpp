...
#include "meeting_service_components_wrap/meeting_raw_archiving_wrap.cpp"
//
...
ZNSDKError ZNativeSDKWrap::InitSDK(ZNInitParam& initParam)
{
	...
	param.strBrandingName = initParam.strBrandingName.c_str();
...
ZoomSTRING ZNativeSDKWrap::GetVersion()
{
	...
	const wchar_t* sdk_version = ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetSDKVersion();
	if (sdk_version)
	{
		zn_version = sdk_version;
	}
	return zn_version;
}
