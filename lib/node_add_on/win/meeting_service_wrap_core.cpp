...
ZNSDKError ZMeetingServiceWrap::Start(ZNStartParam startParam)
{
	...
	param.param.normaluserStart.customer_key = startParam.customer_key.c_str();
...
ZNSDKError ZMeetingServiceWrap::Start_WithoutLogin(ZNStartParam startParam)
{
	...
	param.param.withoutloginStart.customer_key = startParam.customer_key.c_str();
...
ZNSDKError ZMeetingServiceWrap::Join(ZNJoinParam joinParam)
{
	...
	param.param.normaluserJoin.customer_key = joinParam.customer_key.c_str();
	...
	{
		param.param.normaluserJoin.app_privilege_token = joinParam.app_privilege_token.c_str();
	}
	if (!joinParam.join_token.empty())
	{
		param.param.normaluserJoin.join_token = joinParam.join_token.c_str();
	}
...
ZNSDKError ZMeetingServiceWrap::Join_WithoutLogin(ZNJoinParam joinParam)
{
	...
	if (!joinParam.app_privilege_token.empty())
	{
		param.param.withoutloginuserJoin.app_privilege_token = joinParam.app_privilege_token.c_str();
	}
	if (!joinParam.join_token.empty())
	{
		param.param.withoutloginuserJoin.join_token = joinParam.join_token.c_str();
	}
	...
	param.param.withoutloginuserJoin.customer_key = joinParam.customer_key.c_str();
...
ZNSDKError ZMeetingUICtrlWrap::SetMeetingTopic(ZoomSTRING meetingtopic)
{
	ZNSDKError err = Map2WrapDefine(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetUIController().SetMeetingTopic(meetingtopic.c_str()));
	return err;
}
ZNSDKError ZMeetingUICtrlWrap::SetCustomizedCloudRecordingMgrUrl(ZoomSTRING crmURL)
{
	ZNSDKError err = Map2WrapDefine(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetUIController().SetCustomizedCloudRecordingMgrUrl(crmURL.c_str()));
	return err;
}
ZNSDKError ZMeetingUICtrlWrap::SetCustomizedInvitationDomain(ZoomSTRING invitation_domain)
{
	ZNSDKError err = Map2WrapDefine(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetUIController().SetCustomizedInvitationDomain(invitation_domain.c_str()));
	return err;
}
ZNSDKError ZMeetingUICtrlWrap::SetCustomizedPollingUrl(ZoomSTRING pollingURL, bool bCreate)
{
	ZNSDKError err = Map2WrapDefine(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetUIController().SetCustomizedPollingUrl(pollingURL.c_str(), bCreate));
	return err;
}